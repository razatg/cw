...
export async function loadS3IntoPinecode(fileKey:string) {
    ...
    // split and segment the documents
    const documents = await Promise.all(pages.map(prepareDocument));

    //vectorize and embedd individual doc
    const vectors = await Promise.all(documents.flat().map(embedDocument));

    //upload to pinecone
    const client = await getPineconeClient();
    const pineconeIndex = await client.index("context-window");
...
export const truncateStringBuBuytes = (str: string, bytes: number) => {
    ...
    const docs = await splitter.splitDocuments([
        new Document({
            ...
            metadata:{
                ...
                text:truncateStringBuBuytes(pageContent, 36000)
            },
        })
    ]);
    return docs;
